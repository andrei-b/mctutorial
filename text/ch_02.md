...
:pushpin: In Uno both memory types are addressed using 16 bit addresses that start from 0. In mega the flash memory is addressed using a 24 bit addresses (that is how it can use more than 64KB of memory). Keep this in mind when you do the pointer arithmetics. 
