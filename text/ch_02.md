...
The uart_send_char is simple but noe efficient: the program is blocked till all the characters are not sent. When doing real-time programming we often need to be able to perform other actions while writing and reading to and from UART. For this we will need an iterrupt based I/O which we will consider in calter chapters.
